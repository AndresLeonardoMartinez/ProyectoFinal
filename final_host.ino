...
#define PANID 0xcafe
#define HOST_FINAL_ADDRESS 0x4202
...
void cantLux (int valor){
   int valorLDR=valor;
   sprintf(str, "H%dV%d!", HOST_FINAL_ADDRESS,valorLDR);
...
void setup() {
  ...
  mrf.address16_write(HOST_FINAL_ADDRESS);
...
void loop() {
    ...
    if (current_time - last_time > tx_interval) {
        ...
        if (!fallando){
          Serial.println("enviando al serve\n");
          sprintf(str, "H%dV%d!", HOST_FINAL_ADDRESS,temperatura);
          mrf.send16( SERVER_ADDRESS, str);
        }
        else
        {
          Serial.println("enviando al router");
          mrf.send16(ROUTER_ADDRESS, str);
          }
    }
